# 102 — Night Orchestrator Pulse (2026-03-23 02:00 IST)
...
2. **Credentialed auth probe + fallback source continuity task**
   ...
   - Fallback (after >20 min blocker persistence from prior pulses): `npm run -s atom:sources:validate-sync-candidates`
...
## Blocker
- Supabase smoke account credentials in runtime/secret path are still mismatched or stale.
- Failure output:
  - `token: status=400`
