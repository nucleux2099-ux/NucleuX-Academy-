...
import {
  PEDIATRICS_SUBSPECIALTIES,
  PEDIATRICS_UG_MODULES,
  PEDIATRICS_PG_TOPICS,
  PEDIATRICS_SS_SPECIALTIES,
  getTotalUGCompetencies,
  type PediatricsLevel,
  type NMCDomain,
} from "@/lib/data/cbme-pediatrics";
...
function levelBadge(level: PediatricsLevel) {
  const map: Record<PediatricsLevel, string> = {
...
export default function PediatricsCBMEPage() {
    ...
    <div className="ui-shell ui-page space-y-6">
      <div>
        ...
        <h1 className="text-3xl font-bold text-[#E8E0D5]">
          🧒 Pediatrics — CBME Curriculum Map
        </h1>
        <p className="text-[#A0B0BC] mt-1">
          Complete UG → PG → SS progression across {PEDIATRICS_SUBSPECIALTIES.length} subspecialties
        </p>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-[#364A5E] border border-[rgba(232,224,213,0.1)] rounded-lg p-3">
          <div className="text-2xl font-bold text-[#7BA69E]">{totalUG}</div>
          <div className="text-xs text-[#6B7A88]">UG Competencies (PE1–PE34)</div>
        </div>
        <div className="bg-[#364A5E] border border-[rgba(232,224,213,0.1)] rounded-lg p-3">
          <div className="text-2xl font-bold text-[#8FD5D5]">{PEDIATRICS_PG_TOPICS.length}</div>
          <div className="text-xs text-[#6B7A88]">PG Syllabus Topics (MD Pediatrics)</div>
        </div>
        <div className="bg-[#364A5E] border border-[rgba(232,224,213,0.1)] rounded-lg p-3">
          <div className="text-2xl font-bold text-[#E57373]">{PEDIATRICS_SS_SPECIALTIES.length}</div>
          <div className="text-xs text-[#6B7A88]">SS Specialties (DM)</div>
        </div>
        <div className="bg-[#364A5E] border border-[rgba(232,224,213,0.1)] rounded-lg p-3">
          <div className="text-2xl font-bold text-[#E8E0D5]">{PEDIATRICS_SUBSPECIALTIES.length}</div>
      ...
      <input
        ...
        className="w-full max-w-md px-4 py-2 bg-[#364A5E] border border-[rgba(232,224,213,0.1)] rounded-lg text-[#E8E0D5] placeholder:text-[#6B7A88] focus:outline-none focus:ring-2 focus:ring-[#5BB3B3]/45"
      />
      ...
      <div className="flex gap-2 border-b border-[rgba(232,224,213,0.1)] pb-2">
        {(
          [
            ...
            { key: "pg" as ViewTab, label: `🔵 PG (${PEDIATRICS_PG_TOPICS.length} topics)` },
            { key: "ss" as ViewTab, label: `🔴 SS (${PEDIATRICS_SS_SPECIALTIES.length} DM)` },
          ] as const
        ).map((t) => (
          <button
            key={t.key}
            ...
            className={`px-4 py-2 rounded-t-lg text-sm font-medium transition-colors ${
              tab === t.key
                ? "bg-[#364A5E] text-[#E8E0D5] border border-[rgba(232,224,213,0.1)] border-b-transparent"
        ...
        <div className="space-y-4">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {PEDIATRICS_SUBSPECIALTIES.filter(
                  (s) => search === "" || matches(s.name, search) || matches(s.slug, search)
                ).map((s) => (
                  <tr
                    key={s.slug}
                    className="border-b border-[rgba(232,224,213,0.08)] hover:bg-[#364A5E]/50 cursor-pointer transition-colors"
                    onClick={() => {
                  ...
                  >
                    ...
                    <td className="text-right py-2 px-3">
                      <Link
                        href={`/library/pediatrics/${s.slug}`}
          ...
          <div className="bg-[#364A5E] border border-[rgba(232,224,213,0.1)] rounded-lg p-4">
      ...
      {tab === "ug" && (
        <div className="space-y-6">
          <div className="flex items-center gap-2 flex-wrap">
            <button
              ...
              className={`px-3 py-1 rounded text-sm ${
                !selectedSubspec ? "bg-[#7BA69E] text-[#E8E0D5]" : "bg-[#364A5E] text-[#A0B0BC] hover:text-[#E8E0D5]"
              }`}
            >
              All ({PEDIATRICS_UG_MODULES.length} modules)
            </button>
            {PEDIATRICS_SUBSPECIALTIES.filter((s) => s.ugTopicCount > 0).map((s) => (
              <button
                key={s.slug}
                ...
                className={`px-3 py-1 rounded text-sm ${
                  selectedSubspec === s.slug
                    ...
                    : "bg-[#364A5E] text-[#A0B0BC] hover:text-[#E8E0D5]"
                }`}
              >
                {s.icon} {s.name}
              </button>
          ...
          {PEDIATRICS_UG_MODULES.filter((m) => {
            ...
            <div key={mod.id} className="bg-[#364A5E] border border-[rgba(232,224,213,0.1)] rounded-lg p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[#556270] text-xs ml-auto">
                  → {PEDIATRICS_SUBSPECIALTIES.find((s) => s.slug === mod.subspecialty)?.name}
                </span>
              </div>
              ...
              {mod.topicSlugs.length > 0 && (
                <div className="mt-3 flex gap-2 flex-wrap">
                  {mod.topicSlugs.map((slug) => (
                    <Link
                      key={slug}
                      href={`/library/pediatrics/${mod.subspecialty}/${slug}`}
        ...
        <div className="space-y-6">
          <div className="flex items-center gap-2 flex-wrap">
            <button
              onClick={() => setSelectedSubspec(null)}
              className={`px-3 py-1 rounded text-sm ${
                !selectedSubspec ? "bg-[#5BB3B3] text-[#E8E0D5]" : "bg-[#364A5E] text-[#A0B0BC] hover:text-[#E8E0D5]"
              }`}
            >
              All ({PEDIATRICS_PG_TOPICS.length} topics)
            </button>
            {PEDIATRICS_SUBSPECIALTIES.filter((s) => s.pgTopicCount > 0).map((s) => (
              <button
                key={s.slug}
                onClick={() => setSelectedSubspec(s.slug)}
                className={`px-3 py-1 rounded text-sm ${
          ...
          <div className="bg-[#2D3E50] border border-[rgba(232,224,213,0.08)] rounded p-3 text-sm text-[#6B7A88]">
            <span className="text-[#8FD5D5] font-medium">MD Pediatrics</span> — PG syllabus topics grouped by subspecialty.
            Each topic shows which UG modules it builds upon (🟢 → 🔵 progression).
          </div>

          {["General Topics", "Growth & Development", "Neonatology", "Nutrition", "Infectious Diseases", "Systemic Pediatrics"].map((section) => {
            const topics = PEDIATRICS_PG_TOPICS.filter((t) => {
            ...
            return (
              <div key={section}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {topics.map((t) => {
                    const subspec = PEDIATRICS_SUBSPECIALTIES.find((s) => s.slug === t.subspecialty);
                    return (
                      <div key={t.id} className="bg-[#364A5E] border border-[rgba(232,224,213,0.1)] rounded-lg p-4">
                        <div className="flex items-center gap-2 mb-2">
                        ...
                        <div className="mt-2 flex gap-1 flex-wrap">
                          {t.topicSlugs.map((slug) => (
                            <Link
                              key={slug}
                              href={`/library/pediatrics/${t.subspecialty}/${slug}`}
      ...
      {tab === "ss" && (
        <div className="space-y-6">
          <div className="bg-[#2D3E50] border border-[rgba(232,224,213,0.08)] rounded p-3 text-sm text-[#6B7A88]">
            <span className="text-[#E57373] font-medium">DM Super Specialties</span> — Each requires MD Pediatrics as prerequisite.
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {PEDIATRICS_SS_SPECIALTIES.map((ss) => (
              <div
                key={ss.id}
                className="bg-[#364A5E] border border-[rgba(232,224,213,0.1)] rounded-lg p-4 hover:border-[#5BB3B3]/30 transition-colors"
              >
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <div className="mt-3 border-t border-[rgba(232,224,213,0.08)] pt-2">
                    ...
                    <div className="flex gap-1 flex-wrap">
                      {ss.subspecialties.map((slug) => {
                        const sub = PEDIATRICS_SUBSPECIALTIES.find((s) => s.slug === slug);
                        return (
                          <Link
                            key={slug}
                            href={`/library/pediatrics/${slug}`}
          ...
          <div className="bg-[#364A5E] border border-[rgba(232,224,213,0.1)] rounded-lg p-4">
            <h3 className="text-sm font-semibold text-[#C8D2DA] mb-3">Pediatrics Training Progression</h3>
            <div className="flex items-center gap-4 text-sm overflow-x-auto">
              <div className="text-center shrink-0">
                <div className="px-4 py-2 rounded border bg-[#7BA69E]/16 text-[#7BA69E] border-[#7BA69E]/30">
                  ...
                  <div className="text-xs opacity-70">PE1–PE34</div>
              ...
              <div className="text-center shrink-0">
                <div className="px-4 py-2 rounded border bg-[#5BB3B3]/16 text-[#8FD5D5] border-[#5BB3B3]/30">
                  <div className="font-medium">PG (MD Pediatrics)</div>
                  <div className="text-xs opacity-70">3 years</div>
                  <div className="text-xs opacity-70">{PEDIATRICS_PG_TOPICS.length} topics</div>
                </div>
              </div>
              <div className="text-[#556270] text-lg shrink-0">→</div>
              <div className="text-center shrink-0">
                <div className="px-4 py-2 rounded border bg-[#E57373]/16 text-[#E57373] border-[#E57373]/30">
                  ...
                  <div className="text-xs opacity-70">3 years</div>
                  <div className="text-xs opacity-70">{PEDIATRICS_SS_SPECIALTIES.length} specialties</div>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="border-t border-[rgba(232,224,213,0.08)] pt-4 text-xs text-[#556270] flex gap-4 flex-wrap">
        ...
        <span>{PEDIATRICS_PG_TOPICS.length} PG Topics</span>
        <span>{PEDIATRICS_SS_SPECIALTIES.length} SS Specialties</span>
        <span>{PEDIATRICS_SUBSPECIALTIES.length} Subspecialties</span>
