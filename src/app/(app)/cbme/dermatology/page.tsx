  ...
  return (
    <div className="ui-shell ui-page space-y-6">
      <div>
        ...
        <h1 className="text-3xl font-bold text-[#E8E0D5]">🧴 Dermatology, Venereology & Leprosy — CBME Curriculum Map</h1>
        <p className="text-[#A0B0BC] mt-1">Complete UG → PG progression across {DERMATOLOGY_SUBSPECIALTIES.length} subspecialties</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        <div className="bg-[#364A5E]/90 border border-[rgba(232,224,213,0.1)] rounded-lg p-3">
          ...
          <div className="text-xs text-[#6B7A88]">UG Competencies (DR1–DR16)</div>
        </div>
        <div className="bg-[#364A5E]/90 border border-[rgba(232,224,213,0.1)] rounded-lg p-3">
          <div className="text-2xl font-bold text-[#8FD5D5]">{DERMATOLOGY_PG_TOPICS.length}</div>
          <div className="text-xs text-[#6B7A88]">PG Syllabus Topics (MD Dermatology)</div>
        </div>
        <div className="bg-[#364A5E]/90 border border-[rgba(232,224,213,0.1)] rounded-lg p-3">
          <div className="text-2xl font-bold text-[#E8E0D5]">{DERMATOLOGY_SUBSPECIALTIES.length}</div>
                ...
                {DERMATOLOGY_SUBSPECIALTIES.filter((s) => !search || matches(s.name, search)).map((s) => (
                  <tr key={s.slug} className="border-b border-[rgba(232,224,213,0.08)] hover:bg-[#364A5E]/90 cursor-pointer transition-colors"
                    ...
                    <td className="text-right py-2 px-3">
                      <Link href={`/library/dermatology/${s.slug}`} className="text-xs text-[#8FD5D5] hover:text-[#A5E1E1]" onClick={(e) => e.stopPropagation()}>📚 Open</Link>
                    </td>
                  </tr>
                ))}</tbody>
            </table>
          ...
          {DERMATOLOGY_UG_MODULES.filter((m) => {
            ...
            <div key={mod.id} className="bg-[#364A5E]/90 border border-[rgba(232,224,213,0.1)] rounded-lg p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[#556270] text-xs ml-auto">→ {DERMATOLOGY_SUBSPECIALTIES.find((s) => s.slug === mod.subspecialty)?.name}</span>
              </div>
      ...
      {tab === "pg" && (
        <div className="space-y-6">
          ...
          <div className="bg-[#364A5E]/70 border border-[rgba(232,224,213,0.08)] rounded p-3 text-sm text-[#6B7A88]">
            <span className="text-[#8FD5D5] font-medium">MD Dermatology</span> — PG syllabus topics grouped by subspecialty.
          </div>
