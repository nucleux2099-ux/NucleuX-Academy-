...
import {
  DERMATOLOGY_SUBSPECIALTIES,
  DERMATOLOGY_UG_MODULES,
  DERMATOLOGY_PG_TOPICS,
  getTotalUGCompetencies,
} from "@/lib/data/cbme-dermatology";
import type { NMCDomain } from "@/lib/data/cbme-dermatology";
...
export default function DermatologyCBMEPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-6">
      <div>
        ...
        <h1 className="text-3xl font-bold text-white">🧴 Dermatology, Venereology & Leprosy — CBME Curriculum Map</h1>
        <p className="text-zinc-400 mt-1">Complete UG → PG progression across {DERMATOLOGY_SUBSPECIALTIES.length} subspecialties</p>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        <div className="bg-zinc-800/50 border border-zinc-700 rounded-lg p-3">
          <div className="text-2xl font-bold text-green-400">{totalUG}</div>
          <div className="text-xs text-zinc-500">UG Competencies (DR1–DR16)</div>
        </div>
        <div className="bg-zinc-800/50 border border-zinc-700 rounded-lg p-3">
          <div className="text-2xl font-bold text-blue-400">{DERMATOLOGY_PG_TOPICS.length}</div>
          <div className="text-xs text-zinc-500">PG Syllabus Topics (MD Dermatology)</div>
        </div>
        <div className="bg-zinc-800/50 border border-zinc-700 rounded-lg p-3">
          <div className="text-2xl font-bold text-white">{DERMATOLOGY_SUBSPECIALTIES.length}</div>
      ...
      <div className="flex gap-2 border-b border-zinc-700 pb-2">
        {([
          ...
          { key: "pg" as ViewTab, label: `🔵 PG (${DERMATOLOGY_PG_TOPICS.length} topics)` },
        ] as const).map((t) => (
        ...
        <div className="space-y-4">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {DERMATOLOGY_SUBSPECIALTIES.filter((s) => !search || matches(s.name, search)).map((s) => (
                  <tr key={s.slug} className="border-b border-zinc-800 hover:bg-zinc-800/50 cursor-pointer transition-colors"
                    ...
                    <td className="text-right py-2 px-3">
                      <Link href={`/library/dermatology/${s.slug}`} className="text-xs text-blue-400 hover:text-blue-300" onClick={(e) => e.stopPropagation()}>📚 Open</Link>
                    </td>
                  </tr>
                ))}</tbody>
            </table>
        ...
        <div className="space-y-6">
          <div className="flex items-center gap-2 flex-wrap">
            <button onClick={() => setSelectedSubspec(null)}
              ...
              All ({DERMATOLOGY_UG_MODULES.length} modules)
            </button>
            {DERMATOLOGY_SUBSPECIALTIES.filter((s) => s.ugTopicCount > 0).map((s) => (
          ...
          {DERMATOLOGY_UG_MODULES.filter((m) => {
            ...
            <div key={mod.id} className="bg-zinc-800/50 border border-zinc-700 rounded-lg p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-zinc-600 text-xs ml-auto">→ {DERMATOLOGY_SUBSPECIALTIES.find((s) => s.slug === mod.subspecialty)?.name}</span>
              </div>
              ...
              {mod.topicSlugs.length > 0 && (
                <div className="mt-3 flex gap-2 flex-wrap">
                  {mod.topicSlugs.map((slug) => (
                    <Link key={slug} href={`/library/dermatology/${mod.subspecialty}/${slug}`}
        ...
        <div className="space-y-6">
          <div className="flex items-center gap-2 flex-wrap">
            <button onClick={() => setSelectedSubspec(null)}
              ...
              All ({DERMATOLOGY_PG_TOPICS.length} topics)
            </button>
            {DERMATOLOGY_SUBSPECIALTIES.filter((s) => s.pgTopicCount > 0).map((s) => (
              <button key={s.slug} onClick={() => setSelectedSubspec(s.slug)}
          ...
          <div className="bg-zinc-800/30 border border-zinc-700/50 rounded p-3 text-sm text-zinc-500">
            <span className="text-blue-400 font-medium">MD Dermatology</span> — PG syllabus topics grouped by subspecialty.
          </div>
          {[...new Set(DERMATOLOGY_PG_TOPICS.map((t) => t.section))].map((section) => {
            const topics = DERMATOLOGY_PG_TOPICS.filter((t) => {
            ...
            return (
              <div key={section}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {topics.map((t) => {
                    const subspec = DERMATOLOGY_SUBSPECIALTIES.find((s) => s.slug === t.subspecialty);
                    return (
                      <div key={t.id} className="bg-zinc-800/50 border border-zinc-700 rounded-lg p-4">
                        ...
                        <div className="mt-2 flex gap-1 flex-wrap">
                          {t.topicSlugs.map((slug) => (
                            <Link key={slug} href={`/library/dermatology/${t.subspecialty}/${slug}`}
      ...
      <div className="border-t border-zinc-800 pt-4 text-xs text-zinc-600 flex gap-4 flex-wrap">
        <span>16 UG Modules</span>
        <span>{totalUG} UG Competencies</span>
        <span>{DERMATOLOGY_PG_TOPICS.length} PG Topics</span>
        <span>{DERMATOLOGY_SUBSPECIALTIES.length} Subspecialties</span>
