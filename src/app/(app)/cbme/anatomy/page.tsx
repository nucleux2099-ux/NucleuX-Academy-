...
import {
  ANATOMY_SUBSPECIALTIES,
  ANATOMY_UG_MODULES,
  ANATOMY_PG_TOPICS,
  getTotalUGCompetencies,
  type AnatomyLevel,
  type NMCDomain,
} from "@/lib/data/cbme-anatomy";
...
function levelBadge(level: AnatomyLevel) {
  const map: Record<AnatomyLevel, string> = {
...
export default function AnatomyCBMEPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-6">
      <div>
        ...
        <h1 className="text-3xl font-bold text-white">🧬 Anatomy — CBME Curriculum Map</h1>
        <p className="text-zinc-400 mt-1">Complete UG → PG progression across {ANATOMY_SUBSPECIALTIES.length} subspecialties</p>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-zinc-800/50 border border-zinc-700 rounded-lg p-3">
          <div className="text-2xl font-bold text-green-400">{totalUG}</div>
          <div className="text-xs text-zinc-500">UG Competencies (AN1–AN80)</div>
        </div>
        <div className="bg-zinc-800/50 border border-zinc-700 rounded-lg p-3">
          <div className="text-2xl font-bold text-blue-400">{ANATOMY_PG_TOPICS.length}</div>
          <div className="text-xs text-zinc-500">PG Syllabus Topics (MD Anatomy)</div>
        </div>
        <div className="bg-zinc-800/50 border border-zinc-700 rounded-lg p-3">
          <div className="text-2xl font-bold text-white">{ANATOMY_UG_MODULES.length}</div>
          <div className="text-xs text-zinc-500">UG Modules</div>
        </div>
        <div className="bg-zinc-800/50 border border-zinc-700 rounded-lg p-3">
          <div className="text-2xl font-bold text-white">{ANATOMY_SUBSPECIALTIES.length}</div>
      ...
      <div className="flex gap-2 border-b border-zinc-700 pb-2">
        {([
          ...
          { key: "pg" as ViewTab, label: `🔵 PG (${ANATOMY_PG_TOPICS.length} topics)` },
        ] as const).map((t) => (
        ...
        <div className="space-y-4">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {ANATOMY_SUBSPECIALTIES.filter((s) => search === "" || matches(s.name, search)).map((s) => (
                  <tr key={s.slug} className="border-b border-zinc-800 hover:bg-zinc-800/50 cursor-pointer transition-colors" onClick={() => { setSelectedSubspec(s.slug); setTab("ug"); }}>
                    ...
                    <td className="text-center py-2 px-3"><div className="flex gap-1 justify-center">{s.levels.map((l) => (<span key={l} className={`px-1.5 py-0.5 text-xs rounded border ${levelBadge(l)}`}>{l}</span>))}</div></td>
                    <td className="text-right py-2 px-3"><Link href={`/library/anatomy/${s.slug}`} className="text-xs text-blue-400 hover:text-blue-300" onClick={(e) => e.stopPropagation()}>📚 Open</Link></td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="bg-zinc-800/50 border border-zinc-700 rounded-lg p-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              {(["K", "KH", "SH", "P"] as NMCDomain[]).map((d) => { const badge = domainBadge(d); return (<div key={d} className={`px-3 py-2 rounded border ${badge.bg} text-xs`}><div className="font-medium">{badge.label}</div></div>); })}
            </div>
          </div>
        </div>
      )}
      ...
      {tab === "ug" && (
        <div className="space-y-6">
          <div className="flex items-center gap-2 flex-wrap">
            <button onClick={() => setSelectedSubspec(null)} className={`px-3 py-1 rounded text-sm ${!selectedSubspec ? "bg-green-600 text-white" : "bg-zinc-800 text-zinc-400 hover:text-white"}`}>All ({ANATOMY_UG_MODULES.length} modules)</button>
            {ANATOMY_SUBSPECIALTIES.filter((s) => s.ugTopicCount > 0).map((s) => (
              <button key={s.slug} onClick={() => setSelectedSubspec(s.slug)} className={`px-3 py-1 rounded text-sm ${selectedSubspec === s.slug ? "bg-green-600 text-white" : "bg-zinc-800 text-zinc-400 hover:text-white"}`}>{s.icon} {s.name}</button>
            ))}
          </div>
          {ANATOMY_UG_MODULES.filter((m) => {
            ...
            <div key={mod.id} className="bg-zinc-800/50 border border-zinc-700 rounded-lg p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-zinc-600 text-xs ml-auto">→ {ANATOMY_SUBSPECIALTIES.find((s) => s.slug === mod.subspecialty)?.name}</span>
              </div>
              ...
              {mod.topicSlugs.length > 0 && (
                <div className="mt-3 flex gap-2 flex-wrap">
                  {mod.topicSlugs.map((slug) => (
                    <Link key={slug} href={`/library/anatomy/${mod.subspecialty}/${slug}`} className="text-xs px-2 py-1 rounded bg-zinc-700/50 text-blue-400 hover:text-blue-300 hover:bg-zinc-700">📚 {slug.replace(/-/g, " ")}</Link>
                  ))}
                </div>
              )}
            </div>
      ...
      {tab === "pg" && (
        <div className="space-y-6">
          <div className="flex items-center gap-2 flex-wrap">
            <button onClick={() => setSelectedSubspec(null)} className={`px-3 py-1 rounded text-sm ${!selectedSubspec ? "bg-blue-600 text-white" : "bg-zinc-800 text-zinc-400 hover:text-white"}`}>All ({ANATOMY_PG_TOPICS.length} topics)</button>
            {ANATOMY_SUBSPECIALTIES.filter((s) => s.pgTopicCount > 0).map((s) => (
              <button key={s.slug} onClick={() => setSelectedSubspec(s.slug)} className={`px-3 py-1 rounded text-sm ${selectedSubspec === s.slug ? "bg-blue-600 text-white" : "bg-zinc-800 text-zinc-400 hover:text-white"}`}>{s.icon} {s.name}</button>
            ))}
          </div>
          <div className="bg-zinc-800/30 border border-zinc-700/50 rounded p-3 text-sm text-zinc-500">
            <span className="text-blue-400 font-medium">MD Anatomy</span> — PG syllabus topics grouped by subspecialty.
          </div>
          {["General", "Regional", "Research"].map((section) => {
            const topics = ANATOMY_PG_TOPICS.filter((t) => {
            ...
            return (
              <div key={section}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {topics.map((t) => {
                    const subspec = ANATOMY_SUBSPECIALTIES.find((s) => s.slug === t.subspecialty);
                    return (
                      <div key={t.id} className="bg-zinc-800/50 border border-zinc-700 rounded-lg p-4">
                        ...
                        {t.ugModuleRefs.length > 0 && (
                          <div className="mt-2 flex gap-1 flex-wrap">
                            ...
                            {t.ugModuleRefs.map((ref) => (<span key={ref} className="px-1.5 py-0.5 text-xs rounded bg-green-600/10 text-green-500 border border-green-500/20 font-mono">{ref}</span>))}
                          </div>
                        )}
                        <div className="mt-2 flex gap-1 flex-wrap">
                          {t.topicSlugs.map((slug) => (
                            <Link key={slug} href={`/library/anatomy/${t.subspecialty}/${slug}`} className="text-xs px-2 py-0.5 rounded bg-zinc-700/50 text-blue-400 hover:text-blue-300">{slug.replace(/-/g, " ")}</Link>
                          ))}
                        </div>
                      </div>
                    );
      ...
      <div className="border-t border-zinc-800 pt-4 text-xs text-zinc-600 flex gap-4 flex-wrap">
        <span>{ANATOMY_UG_MODULES.length} UG Modules</span>
        <span>{totalUG} UG Competencies</span>
        <span>{ANATOMY_PG_TOPICS.length} PG Topics</span>
        <span>{ANATOMY_SUBSPECIALTIES.length} Subspecialties</span>
