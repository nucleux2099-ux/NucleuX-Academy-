  ...
  return (
    <div className="ui-shell ui-page space-y-6">
      <div>
        ...
        <h1 className="text-3xl font-bold text-[#E8E0D5]">🧬 Anatomy — CBME Curriculum Map</h1>
        <p className="text-[#A0B0BC] mt-1">Complete UG → PG progression across {ANATOMY_SUBSPECIALTIES.length} subspecialties</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-[#364A5E]/90 border border-[rgba(232,224,213,0.1)] rounded-lg p-3">
          ...
          <div className="text-xs text-[#6B7A88]">UG Competencies (AN1–AN80)</div>
        </div>
        <div className="bg-[#364A5E]/90 border border-[rgba(232,224,213,0.1)] rounded-lg p-3">
          <div className="text-2xl font-bold text-[#8FD5D5]">{ANATOMY_PG_TOPICS.length}</div>
          <div className="text-xs text-[#6B7A88]">PG Syllabus Topics (MD Anatomy)</div>
        </div>
        <div className="bg-[#364A5E]/90 border border-[rgba(232,224,213,0.1)] rounded-lg p-3">
          <div className="text-2xl font-bold text-[#E8E0D5]">{ANATOMY_UG_MODULES.length}</div>
          <div className="text-xs text-[#6B7A88]">UG Modules</div>
        </div>
        <div className="bg-[#364A5E]/90 border border-[rgba(232,224,213,0.1)] rounded-lg p-3">
          <div className="text-2xl font-bold text-[#E8E0D5]">{ANATOMY_SUBSPECIALTIES.length}</div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {ANATOMY_SUBSPECIALTIES.filter((s) => search === "" || matches(s.name, search)).map((s) => (
                  <tr key={s.slug} className="border-b border-[rgba(232,224,213,0.08)] hover:bg-[#364A5E]/90 cursor-pointer transition-colors" onClick={() => { setSelectedSubspec(s.slug); setTab("ug"); }}>
                    ...
                    <td className="text-right py-2 px-3"><Link href={`/library/anatomy/${s.slug}`} className="text-xs text-[#8FD5D5] hover:text-[#A5E1E1]" onClick={(e) => e.stopPropagation()}>📚 Open</Link></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {tab === "ug" && (
        <div className="space-y-6">
          <div className="flex items-center gap-2 flex-wrap">
            <button onClick={() => setSelectedSubspec(null)} className={`px-3 py-1 rounded text-sm ${!selectedSubspec ? "bg-[#7BA69E] text-[#E8E0D5]" : "bg-[#2D3E50] text-[#A0B0BC] hover:text-[#E8E0D5]"}`}>All ({ANATOMY_UG_MODULES.length} modules)</button>
            {ANATOMY_SUBSPECIALTIES.filter((s) => s.ugTopicCount > 0).map((s) => (
              <button key={s.slug} onClick={() => setSelectedSubspec(s.slug)} className={`px-3 py-1 rounded text-sm ${selectedSubspec === s.slug ? "bg-[#7BA69E] text-[#E8E0D5]" : "bg-[#2D3E50] text-[#A0B0BC] hover:text-[#E8E0D5]"}`}>{s.icon} {s.name}</button>
            ))}
          </div>
          {ANATOMY_UG_MODULES.filter((m) => {
            ...
            <div key={mod.id} className="bg-[#364A5E]/90 border border-[rgba(232,224,213,0.1)] rounded-lg p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[#556270] text-xs ml-auto">→ {ANATOMY_SUBSPECIALTIES.find((s) => s.slug === mod.subspecialty)?.name}</span>
              </div>
              ...
              {mod.topicSlugs.length > 0 && (
                <div className="mt-3 flex gap-2 flex-wrap">
                  {mod.topicSlugs.map((slug) => (
                    <Link key={slug} href={`/library/anatomy/${mod.subspecialty}/${slug}`} className="text-xs px-2 py-1 rounded bg-[#3A4D5F]/65 text-[#8FD5D5] hover:text-[#A5E1E1] hover:bg-[#3A4D5F]">📚 {slug.replace(/-/g, " ")}</Link>
                  ))}
                </div>
              )}
            </div>
      ...
      {tab === "pg" && (
        <div className="space-y-6">
          <div className="flex items-center gap-2 flex-wrap">
            <button onClick={() => setSelectedSubspec(null)} className={`px-3 py-1 rounded text-sm ${!selectedSubspec ? "bg-[#5BB3B3] text-[#E8E0D5]" : "bg-[#2D3E50] text-[#A0B0BC] hover:text-[#E8E0D5]"}`}>All ({ANATOMY_PG_TOPICS.length} topics)</button>
            {ANATOMY_SUBSPECIALTIES.filter((s) => s.pgTopicCount > 0).map((s) => (
              <button key={s.slug} onClick={() => setSelectedSubspec(s.slug)} className={`px-3 py-1 rounded text-sm ${selectedSubspec === s.slug ? "bg-[#5BB3B3] text-[#E8E0D5]" : "bg-[#2D3E50] text-[#A0B0BC] hover:text-[#E8E0D5]"}`}>{s.icon} {s.name}</button>
            ))}
          </div>
          <div className="bg-[#364A5E]/70 border border-[rgba(232,224,213,0.08)] rounded p-3 text-sm text-[#6B7A88]">
            <span className="text-[#8FD5D5] font-medium">MD Anatomy</span> — PG syllabus topics grouped by subspecialty.
          </div>
                      ...
                      <div key={t.id} className="bg-[#364A5E]/90 border border-[rgba(232,224,213,0.1)] rounded-lg p-4">
                        ...
                        {t.ugModuleRefs.length > 0 && (
                          <div className="mt-2 flex gap-1 flex-wrap">
                            ...
                            {t.ugModuleRefs.map((ref) => (<span key={ref} className="px-1.5 py-0.5 text-xs rounded bg-[#7BA69E]/14 text-[#7BA69E] border border-[#7BA69E]/24 font-mono">{ref}</span>))}
                          </div>
                        )}
                        <div className="mt-2 flex gap-1 flex-wrap">
                          {t.topicSlugs.map((slug) => (
                            <Link key={slug} href={`/library/anatomy/${t.subspecialty}/${slug}`} className="text-xs px-2 py-0.5 rounded bg-[#3A4D5F]/65 text-[#8FD5D5] hover:text-[#A5E1E1]">{slug.replace(/-/g, " ")}</Link>
                          ))}
                        </div>
                      </div>
                    );
