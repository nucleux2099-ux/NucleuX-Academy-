...
import {
  ORTHOPEDICS_SUBSPECIALTIES,
  ORTHOPEDICS_UG_MODULES,
  ORTHOPEDICS_PG_TOPICS,
  ORTHOPEDICS_SS_SPECIALTIES,
  getTotalUGCompetencies,
  type OrthopedicsLevel,
  type NMCDomain,
} from "@/lib/data/cbme-orthopedics";
...
function levelBadge(level: OrthopedicsLevel) {
  const map: Record<OrthopedicsLevel, string> = {
...
export default function OrthopedicsCBMEPage() {
    ...
    <div className="ui-shell ui-page space-y-6">
      <div>
        ...
        <h1 className="text-3xl font-bold text-[#E8E0D5]">
          🦴 Orthopedics — CBME Curriculum Map
        </h1>
        <p className="text-[#A0B0BC] mt-1">
          Complete UG → PG → SS progression across {ORTHOPEDICS_SUBSPECIALTIES.length} subspecialties
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-[#364A5E] border border-[rgba(232,224,213,0.1)] rounded-lg p-3">
          ...
          <div className="text-xs text-[#6B7A88]">UG Competencies (OR1–OR14)</div>
        </div>
        <div className="bg-[#364A5E] border border-[rgba(232,224,213,0.1)] rounded-lg p-3">
          <div className="text-2xl font-bold text-[#8FD5D5]">{ORTHOPEDICS_PG_TOPICS.length}</div>
          <div className="text-xs text-[#6B7A88]">PG Syllabus Topics (MS Orthopedics)</div>
        </div>
        <div className="bg-[#364A5E] border border-[rgba(232,224,213,0.1)] rounded-lg p-3">
          <div className="text-2xl font-bold text-[#E57373]">{ORTHOPEDICS_SS_SPECIALTIES.length}</div>
          <div className="text-xs text-[#6B7A88]">SS Specialties (MCh)</div>
        </div>
        <div className="bg-[#364A5E] border border-[rgba(232,224,213,0.1)] rounded-lg p-3">
          <div className="text-2xl font-bold text-[#E8E0D5]">{ORTHOPEDICS_SUBSPECIALTIES.length}</div>
      ...
      <div className="flex gap-2 border-b border-[rgba(232,224,213,0.1)] pb-2">
        {(
          [
            ...
            { key: "pg" as ViewTab, label: `🔵 PG (${ORTHOPEDICS_PG_TOPICS.length} topics)` },
            { key: "ss" as ViewTab, label: `🔴 SS (${ORTHOPEDICS_SS_SPECIALTIES.length} MCh)` },
          ] as const
        ).map((t) => (
          <button
            key={t.key}
        ...
        <div className="space-y-4">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {ORTHOPEDICS_SUBSPECIALTIES.filter(
                  ...
                  >
                    ...
                    <td className="text-right py-2 px-3">
                      <Link
                        href={`/library/orthopedics/${s.slug}`}
      ...
      {tab === "ug" && (
        <div className="space-y-6">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            >
              All ({ORTHOPEDICS_UG_MODULES.length} modules)
            </button>
            {ORTHOPEDICS_SUBSPECIALTIES.filter((s) => s.ugTopicCount > 0).map((s) => (
              <button
                key={s.slug}
          ...
          {ORTHOPEDICS_UG_MODULES.filter((m) => {
            ...
            <div key={mod.id} className="bg-[#364A5E] border border-[rgba(232,224,213,0.1)] rounded-lg p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[#556270] text-xs ml-auto">
                  → {ORTHOPEDICS_SUBSPECIALTIES.find((s) => s.slug === mod.subspecialty)?.name}
                </span>
              </div>
              ...
              {mod.topicSlugs.length > 0 && (
                <div className="mt-3 flex gap-2 flex-wrap">
                  {mod.topicSlugs.map((slug) => (
                    <Link
                      key={slug}
                      href={`/library/orthopedics/${mod.subspecialty}/${slug}`}
      ...
      {tab === "pg" && (
        <div className="space-y-6">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            >
              All ({ORTHOPEDICS_PG_TOPICS.length} topics)
            </button>
            {ORTHOPEDICS_SUBSPECIALTIES.filter((s) => s.pgTopicCount > 0).map((s) => (
              <button
                key={s.slug}
                onClick={() => setSelectedSubspec(s.slug)}
                className={`px-3 py-1 rounded text-sm ${
          ...
          <div className="bg-[#2D3E50] border border-[rgba(232,224,213,0.08)] rounded p-3 text-sm text-[#6B7A88]">
            <span className="text-[#8FD5D5] font-medium">MS Orthopedics</span> — PG syllabus topics grouped by subspecialty.
            Each topic shows which UG modules it builds upon (🟢 → 🔵 progression).
          </div>

          {["General Topics", "Trauma", "Infections", "Tumors", "Spine", "Arthroplasty", "Pediatric Orthopedics", "Sports Medicine"].map((section) => {
            const topics = ORTHOPEDICS_PG_TOPICS.filter((t) => {
            ...
            return (
              <div key={section}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {topics.map((t) => {
                    const subspec = ORTHOPEDICS_SUBSPECIALTIES.find((s) => s.slug === t.subspecialty);
                    return (
                      <div key={t.id} className="bg-[#364A5E] border border-[rgba(232,224,213,0.1)] rounded-lg p-4">
                        ...
                        <div className="mt-2 flex gap-1 flex-wrap">
                          {t.topicSlugs.map((slug) => (
                            <Link
                              key={slug}
                              href={`/library/orthopedics/${t.subspecialty}/${slug}`}
      ...
      {tab === "ss" && (
        <div className="space-y-6">
          <div className="bg-[#2D3E50] border border-[rgba(232,224,213,0.08)] rounded p-3 text-sm text-[#6B7A88]">
            <span className="text-[#E57373] font-medium">MCh Super Specialties</span> — Each requires MS Orthopedics as prerequisite.
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {ORTHOPEDICS_SS_SPECIALTIES.map((ss) => (
              <div
                key={ss.id}
                  ...
                  <div className="mt-3 border-t border-[rgba(232,224,213,0.08)] pt-2">
                    ...
                    <div className="flex gap-1 flex-wrap">
                      {ss.subspecialties.map((slug) => {
                        const sub = ORTHOPEDICS_SUBSPECIALTIES.find((s) => s.slug === slug);
                        return (
                          <Link
                            key={slug}
                            href={`/library/orthopedics/${slug}`}
          ...
          <div className="bg-[#364A5E] border border-[rgba(232,224,213,0.1)] rounded-lg p-4">
            <h3 className="text-sm font-semibold text-[#C8D2DA] mb-3">Orthopedics Training Progression</h3>
            <div className="flex items-center gap-4 text-sm overflow-x-auto">
              <div className="text-center shrink-0">
                <div className="px-4 py-2 rounded border bg-[#7BA69E]/16 text-[#7BA69E] border-[#7BA69E]/30">
                  ...
                  <div className="text-xs opacity-70">OR1–OR14</div>
              ...
              <div className="text-center shrink-0">
                <div className="px-4 py-2 rounded border bg-[#5BB3B3]/16 text-[#8FD5D5] border-[#5BB3B3]/30">
                  <div className="font-medium">PG (MS Orthopedics)</div>
                  <div className="text-xs opacity-70">3 years</div>
                  <div className="text-xs opacity-70">{ORTHOPEDICS_PG_TOPICS.length} topics</div>
                </div>
              </div>
              <div className="text-[#556270] text-lg shrink-0">→</div>
              <div className="text-center shrink-0">
                <div className="px-4 py-2 rounded border bg-[#E57373]/16 text-[#E57373] border-[#E57373]/30">
                  ...
                  <div className="text-xs opacity-70">3 years</div>
                  <div className="text-xs opacity-70">{ORTHOPEDICS_SS_SPECIALTIES.length} specialties</div>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="border-t border-[rgba(232,224,213,0.08)] pt-4 text-xs text-[#556270] flex gap-4 flex-wrap">
        ...
        <span>{ORTHOPEDICS_PG_TOPICS.length} PG Topics</span>
        <span>{ORTHOPEDICS_SS_SPECIALTIES.length} SS Specialties</span>
        <span>{ORTHOPEDICS_SUBSPECIALTIES.length} Subspecialties</span>
