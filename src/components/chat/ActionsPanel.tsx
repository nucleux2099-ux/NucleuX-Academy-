...
import {
  FileText, Brain, Presentation, Headphones, Layers, Copy, Download, Sparkles,
} from "lucide-react";
import type { OutputCard, Message } from "./data";
  ...
  return (
    <Tabs defaultValue={defaultTab} className="flex flex-col h-full">
      <div className="p-3 border-b border-[#1E3A5F]">
        <div className="rounded-xl bg-gradient-to-r from-[#5BB3B3]/10 via-[#93C5FD]/10 to-[#A78BFA]/10 border border-[#1E3A5F] p-2.5">
          <h2 className="text-sm font-semibold text-white flex items-center gap-2">
            <Sparkles className="w-4 h-4 text-[#5EEAD4]" /> Workspace
          </h2>
          <TabsList className="grid w-full mt-2 grid-cols-2 bg-[#162535] border border-[#1E3A5F]">
            <TabsTrigger value="actions" className="text-xs data-[state=active]:bg-[#5BB3B3]/20 data-[state=active]:text-white">Actions</TabsTrigger>
            <TabsTrigger value="canvas" className="text-xs data-[state=active]:bg-[#5BB3B3]/20 data-[state=active]:text-white">Canvas</TabsTrigger>
          </TabsList>
        </div>
      </div>

      <TabsContent value="actions" className="mt-0 flex-1 flex flex-col data-[state=inactive]:hidden">
      ...
      <TabsContent value="canvas" className="mt-0 flex-1 data-[state=inactive]:hidden">
        <CanvasPanel
          messages={messages}
          selectedCodeBlockId={selectedCodeBlockId}
          onSelectCodeBlock={(id) => onSelectCodeBlock?.(id)}
        />
      </TabsContent>
    </Tabs>
  );
