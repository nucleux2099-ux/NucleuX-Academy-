...
export function StreakCard() {
  ...
  return (
    <Card className="bg-gradient-to-br from-[#3A4D5F] to-[#2D3E50] border-[rgba(245,158,11,0.2)]">
      <CardContent className="p-6">
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <Flame className={cn(
                "w-6 h-6",
                currentStreak > 0 ? "text-[#C9A86C]" : "text-[#6B7280]"
              )} />
              <span className="text-sm text-[#A0B0BC]">Current Streak</span>
            </div>
            <p className="text-4xl font-bold text-[#E8E0D5]">
              {currentStreak}
              <span className="text-lg text-[#A0B0BC] ml-1">days</span>
            </p>
          </div>
        ...
        {currentStreak > 0 && (
          <div className="mt-4 p-3 bg-[rgba(245,158,11,0.1)] rounded-lg">
            <p className="text-sm text-[#C9A86C]">
              {currentStreak >= 7 
      ...
      <CardContent className="p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium text-[#E8E0D5]">Daily Goal</span>
          </div>
          <Badge className={cn(
            progress >= 100 
        ...
        {progress >= 100 ? (
          ...
          <p className="text-sm text-[#A0B0BC] mt-3">
...
export function ExamCountdownCard() {
  ...
  return (
    <Card className={cn(
      "border-2",
      daysLeft <= 30 
        ? "bg-[rgba(239,68,68,0.1)] border-[#E57373]" 
        : daysLeft <= 90
          ? "bg-[rgba(245,158,11,0.1)] border-[#C9A86C]"
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              <Calendar className={cn(
                "w-5 h-5",
                daysLeft <= 30 ? "text-[#E57373]" : daysLeft <= 90 ? "text-[#C9A86C]" : "text-[#5BB3B3]"
              )} />
              <span className="text-sm text-[#A0B0BC]">{examConfig.label}</span>
            </div>
            <p className={cn(
              "text-3xl font-bold",
              daysLeft <= 30 ? "text-[#E57373]" : daysLeft <= 90 ? "text-[#C9A86C]" : "text-[#E8E0D5]"
            )}>
              {daysLeft}
              <span className="text-lg text-[#A0B0BC] ml-1">days left</span>
            </p>
          </div>
...
export function WeakAreasCard() {
  ...
  return (
    <Card className="bg-[#3A4D5F] border-[rgba(239,68,68,0.2)]">
      <CardHeader className="pb-2">
        <CardTitle className="text-[#E8E0D5] flex items-center gap-2 text-base">
        ...
        <div className="space-y-2">
          {weakAreas.slice(0, 3).map((area) => (
            <Link
              ...
              className="flex items-center justify-between p-3 bg-[#2D3E50] rounded-lg hover:bg-[rgba(239,68,68,0.05)] transition-colors"
            >
              <div>
                <p className="text-sm font-medium text-[#E8E0D5]">{area.topicName}</p>
...
export function DueForReviewCard() {
  ...
  return (
    <Card className="bg-[#3A4D5F] border-[rgba(139,92,246,0.2)]">
      <CardHeader className="pb-2">
        <CardTitle className="text-[#E8E0D5] flex items-center gap-2 text-base">
        ...
        <div className="space-y-2">
          {dueTopics.slice(0, 3).map((topicId) => (
            <Link
              ...
              className="flex items-center justify-between p-3 bg-[#2D3E50] rounded-lg hover:bg-[rgba(139,92,246,0.05)] transition-colors"
            >
              <p className="text-sm font-medium text-[#E8E0D5]">{topicId}</p>
...
export function LearningStatsOverview() {
  ...
  return (
    <Card className="bg-gradient-to-br from-[#3A4D5F] to-[#2D3E50] border-[rgba(91,179,179,0.2)]">
      <CardContent className="p-6">
        <div className="flex items-center gap-4 mb-6">
          <div className="w-12 h-12 rounded-full bg-gradient-to-br from-[#5BB3B3] to-[#8B5CF6] flex items-center justify-center text-2xl">
            {levelConfig.icon}
          </div>
          <div>
            <p className="font-semibold text-[#E8E0D5]">{levelConfig.label}</p>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="p-4 bg-[#2D3E50] rounded-xl">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs text-[#A0B0BC]">Topics</span>
            </div>
            <p className="text-2xl font-bold text-[#E8E0D5]">{user.completedTopicsCount}</p>
          ...
          <div className="p-4 bg-[#2D3E50] rounded-xl">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs text-[#A0B0BC]">Time</span>
            </div>
            <p className="text-2xl font-bold text-[#E8E0D5]">{Math.round(user.totalStudyMinutes / 60)}</p>
            <p className="text-xs text-[#6B7280]">hours studied</p>
          </div>
          
          <div className="p-4 bg-[#2D3E50] rounded-xl">
            <div className="flex items-center gap-2 mb-2">
              <Flame className="w-4 h-4 text-[#C9A86C]" />
              <span className="text-xs text-[#A0B0BC]">Streak</span>
            </div>
            <p className="text-2xl font-bold text-[#E8E0D5]">{user.streak.currentStreak}</p>
          ...
          <div className="p-4 bg-[#2D3E50] rounded-xl">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs text-[#A0B0BC]">Progress</span>
            </div>
            <p className="text-2xl font-bold text-[#E8E0D5]">{Object.keys(user.topicProgress).length}</p>
